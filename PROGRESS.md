...
- **2026-09-16 — v4.3(`ifda.__version__` 2.7.0 → 2.8.0):网页界面的信息架构、列表密度与
  文字对比度。**

  只改 `service/web/index.html` 一个文件,不涉及 API、数据结构和 Python 分析核心。前端是
  `//go:embed web/*` 打进二进制的,所以必须重新 `go build` 才会生效。

  **版本号与去重缓存的副作用**:`ifda.__version__` 从 2.7.0 升到 2.8.0 以标记这个版本。要注意
  这个字符串是拌进 `dedupKey` 的(见 `service/job.go` 里 `Store.analyzerVersion` 的注释),所以
  升版会让 `.data/reports.db` 里所有既有报告被判定为过时:同一个目标再次提交时不再命中缓存
  CopyJob,而是从头跑一遍完整分析。和 v4.1 那次不同——那次升版是**为了**让缓存失效,因为报告
  结构真的变了(新增仪表盘/diff 字段、函数指纹);这次报告 JSON 一个字节都没变,只是渲染方式
  变了,所以这轮重扫拿不到任何新数据,纯粹是时间成本(实测大目标单次分析可达小时级,超时上限
  就是为此放宽到 2 小时的)。如果更希望保住缓存,把这个值改回 2.7.0 即可,不影响本次任何界面
  改动。

  **页头折行**:原页头在一个会换行的 flex 行里塞了 wordmark、四个词的标语、摘要字符串、一个
  `flex:1` 的姓名输入框和六个同等权重的按钮。1600px 下标语被挤成四行,输入框吃掉约 700px——
  而那个值一个会话只填一次。根因是页头把"一次性设置"和"每屏都要看的信息"放在了同一优先级上。
  现在标语回归登录页,页头钉住的是当前报告对应的镜像名加架构/内核/体积;四个工具按钮并成一组
  分段控件;姓名、语言、主题收进用户菜单。

  **十一个标签排不下**:1600px 下 Services 被从中间裁断,AI 分析整个在屏幕外,而且没有任何
  溢出提示——不是样式问题,是顶层导航承载了本该分层的东西。Binaries/Scripts/Components/
  Files/BusyBox/Services 六个视图收到 **Inventory** 一个标签下,两个字符串视图收到 **Strings**
  下,各自用一条二级分段控件,只在所属标签激活时出现。顶层剩五个。

  **仪表盘缺少主次**:五张等大卡片让"总数""严重""高危""二进制数""CVE 数"权重相同,而且只有
  严重和高危两档有数字,中危/低危/提示三档在仪表盘上根本不存在。现在一张宽卡承载总数和一条
  按比例分段的严重度条,五档全部计数(真实数据下中危 919、低危 1554、提示 49 这三档是第一次
  在仪表盘上露出);其余清单计数降到四分之三尺寸的小块行。

  **发现列表的重复**:平铺列表一条发现一张卡,严重度排序下同一组件的 CVE 天然连在一起,实测
  一屏被 48 条连号的 tcpdump CVE 占满。现在按组件把当前页分组——发现是服务端分页的,所以分的
  是"你正在看的这一页",而重复恰恰就发生在这一页内。组头带条数和严重度分布,超过 5 条的组默认
  折叠;组内行去掉组头已经写明的组件前缀。严重度筛选原本是五个没有标签的彩色圆点(C/H/M/L/I),
  改成带名称和该档计数的胶囊。

  **二进制表行高失控**:加固列把六个宽度不一的 chip 换行排成三行,CVE 列把所有 id 内联铺开,
  busybox 那一行因此超过 200px 高,列与列完全对不齐。加固改成固定六格(NX/CN/RL/PI/FT/SY),
  顺序恒定并配图例;CVE 收成可展开的计数;行高锁死 46px。**MD5 保持完整 32 位显示**并给了固定
  列宽——这是这张表里真正会被复制出去的值。路径列改成从左侧截断共享的目录前缀,文件名永不截断
  (此前从右截断,而固件里所有二进制共享同一个长前缀,导致整列每一行都显示成完全相同的
  `/path/to/extracted/rootfs…`,真正区分行的文件名反而被切掉了)。

  **任务卡溢出到右侧报告区**:运行中任务的 `detail` 字段是正在扫描的文件全路径,它被放在一个
  `flex-wrap:wrap` 的 meta 行里且没有任何宽度约束,于是直接画过左栏分隔线、盖在报告面板上。
  这是真实的渲染 bug,不只是观感问题。现在卡片内每个字段都单行裁剪,完整值挂在 title 上。

  **对比度不达标**:`--muted` 承载着元信息行、日志时间戳、表头和所有说明文字,默认主题下
  `#64748b` 压在 `--panel`(`#111827`)上实测 3.4:1,低于正文 4.5:1 的门槛。七套主题逐个按各自
  的面板底色重算,都提到 4.5:1 以上(midnight `#7c8da5` 实测 4.9:1);原值移到新增的 `--dim`,
  只用于分隔线和装饰图标,不再承载文字。浅色主题原值本来就是 4.86:1,未改动。

  **验证**:用真实固件报告验证,不是造数据。在空闲端口跑了一份 `service/.data` 的隔离副本
  (临时账号登录,jobs 目录软链只读),选 job-65(3010 条发现 / 537 个二进制 / 548 个 CVE),
  用无头 Chrome 以 1600x1000、2 倍像素密度对仪表盘、发现、清单、CVE 数据库四个界面逐个截图
  核对,暗色(midnight)和浅色(light)两套主题都走了一遍,控制台无报错;另外用 CDP 点击验证了
  折叠组能正常展开。验证用的进程、临时二进制和数据副本事后已全部清理。

- **2026-09-11 — v4.2:固件识别与解包(FR-ING/FR-EXT),集成 [moria](https://github.com/nmatt0/moria)。**

  之前 `analyze` 一直假设"固件已经解包好",解包这一步默认是外部手动完成的。现在服务层加了
  一个新的任务类型"解包"(`kind:"extract"`),对着一个原始固件镜像(整块 flash dump、分区
  镜像、厂商升级包)跑,内部调用 `moria`(C++20,MIT 协议,自包含二进制,无需 root——
  外部可选工具,和 Ghidra/cve-bin-tool 一样的优雅降级姿态:没装只影响这一种任务类型,不影响
  `analyze`)。任务完成后拿到的不是 findings 报告,而是一份 `regions` 列表(每个区域的偏移量、
  类型、置信度、解包结果状态、解包后路径、文件/目录数,以及 moria 报告的任何警告——即使某个
  区域没能解包成功也会带着原因列出来,不是静默丢弃)。

  **不自动挑"哪个是 rootfs"**:真实的多分区固件经常同时有好几个文件系统区域(双 bank A/B
  镜像、bootloader+kernel+rootfs 挤在一个 dump 里),猜错了会悄悄分析一个没在跑的备用 bank
  而不是真正在用的那个——这个决定交给人工,在网页上看区域列表自己选。每个可用区域旁边有两个
  按钮:"提交为分析任务"(区域已经是解包出来的真实目录树)和"再次解包"(moria 有时候只解一层
  ——比如解出一个容器但里面还嵌套着一层它没认出来的压缩流,这时候对刚解包出来的这个文件/目录
  再跑一次 extract 即可,不需要特殊代码路径,复用的是同一个"解包"任务类型)。

  **moria 以 git 子模块形式随仓库附带**(`moria/`,锁定到具体 commit),用
  `scripts/build-moria.sh` 构建到 `moria/build/moria`。`ifda/ingest` 的查找顺序是
  `$IFDA_MORIA` → 仓库内构建产物 → `PATH` 上的 `moria`。这样做的直接原因:开发这台机器上
  `/usr/local/bin/moria` 是 0.1.0,而仓库锁定的 commit 构建出来是 0.2.1——在加子模块之前,
  项目一直在悄悄用着比锁定版本更旧的解包器,而且从任何地方都看不出来。子模块和构建都不是
  必需的:系统里装了 moria 照样能用,两者都没有时"解包"任务报"不可用",不影响其他任何功能
  (NFR-USE-1)。服务启动时会打印实际解析到的是哪个构建(`moria firmware extraction: moria
  0.2.1`),`/healthz` 增加 `moria_version` 字段,网页据此直接禁用"解包"按钮并给出提示——
  此前按钮始终可点,缺 moria 只能等任务提交后失败才知道。给 `moria_path()` 的三条查找路径
  都补了测试,其中一条专门钉住 `_VENDORED` 的相对路径推导:这个模块一旦挪位置,它会指向一个
  看起来人畜无害的错误路径,然后仓库内构建就再也不会被用到,且没有任何报错。

  **技术实现**:`ifda/ingest` 薄薄包一层 `moria -j -e`,把 moria 自己的"识别 findings" JSON
  和"解包 manifest" JSON(offset 对齐)合并成一份干净的区域列表,`status` 为 `"ok"`/`"partial"`
  且有实际文件/目录产出的才标 `usable`。Go 层新增 `Job.Kind`/`Job.Regions` 字段,复用现有的
  任务队列/进度日志/SSE 这一整套基础设施(解包任务的 `@@IFDA@@` 进度事件走同一条协议),不
  参与已有的去重缓存(解包够快,缓存收益不大,而且缓存机制本来就是围着 SQLite 里的 findings
  报告设计的,解包任务没有那种行)。用真实 moria 二进制 + 一个真实的多格式固件镜像做过手工
  验证(找到的区域、置信度、解包成功/失败原因和 moria 自己报的一致),另外写了走真实 moria
  子进程的自动化测试(合成的小 tar 包,不依赖任何本地固件样本)。

- **2026-09-09 — 分析子进程超时改为可配置(`-analyze-timeout`),默认 30 分钟 → 2 小时。**

  一个二进制数量异常多(数千个)的大体积目标(含大量第三方大型组件)在 `disassemble` 阶段跑到
  将近一半就被杀掉,`job.Error` 记录为 `signal: killed`。根因不是分析逻辑出错,是
  `service/job.go` 里给整个分析子进程写死的 30 分钟 `context.WithTimeout`——这个值只是防止
  真正卡死的安全网,不是按目标大小估算出来的,数千个二进制、其中不乏体积很大的可执行文件时
  完全不够用。现在改成 `-analyze-timeout` 启动参数,默认放宽到 2 小时;没有做"按二进制数量
  动态计算超时"这种更复杂的方案,因为动态估算需要在子进程启动前单独做一次目录扫描,还要和
  Python 侧的 ELF 探测逻辑保持一致,维护成本换不来多少实际收益——固定但可调的上限已经够用。

- **2026-08-25 — v4.1(`ifda.__version__` 2.6.0 → 2.7.0):根目录占比图、对比扫描函数级
  diff、扫描进度准确性。**

  **仪表盘新增根文件系统目录占比图。** 按顶层目录(`/usr`、`/etc`、`/root` 等)展示磁盘占用
  分布,标注版甜甜圈图:扇形内标目录名,扇形外用引线标百分比+格式化后的存储大小(占比过小
  的扇区——真实固件根目录常见十几个顶层目录,大部分很小——只上色不加外部文字标注,避免堆成
  一团反而看不清;旁边的 HTML 图例本来就完整列出了每个目录的精确数值,不算信息损失)。渲染
  优先走 CUDA 逐像素光栅化(每个 GPU 线程从极坐标独立算出自己那个像素属于哪个扇形),没有
  可用设备时退回 Pillow 做 CPU 矢量渲染;文字标注不管走哪条渲染路径,统一在 CPU 侧用 Pillow
  叠加,因为字体光栅化本来就不适合塞进 CUDA kernel。用真实固件在真实 CUDA 设备上跑通,CPU
  兜底路径也验证过。

  **对比扫描新增函数级 diff。** 在原有的基于 MD5 的文件级 diff 之外,反汇编阶段现在会给每个
  函数算一个内容指纹(函数体内所有指令的助记符直方图哈希,复用已有的、本来就要抽取调用边的
  那一遍指令遍历,不额外增加反汇编开销),前端对两次扫描里路径相同的二进制按函数名匹配做
  diff:新增/删除/修改/未变化,可视化复用同一套 CUDA/CPU 渲染管线(饼图渲染器已泛化为接受任意
  命名数值分片,不再局限于目录字节数)。这是按函数名匹配,不是 BinDiff 那种结构匹配——一个
  只是被改名或重新去符号的函数,会显示为一增一减,不是"已修改"。

  **扫描进度准确性修复。** `--decompile` 模式下 Ghidra 反编译阶段的进度百分比之前会明显
  倒退(该阶段开始前的一个固定收尾阶段已经报了 99%,decompile 自己的区间却是从更低的数字
  起步),然后在剩余时间里几乎不动——因为 decompile 可能多达数百个目标,却被压缩进固定 3 个
  百分点的显示区间,和目标数量无关。现在整条阶段百分比曲线端到端单调递增,decompile 分到
  的区间也按比例放宽了不少。有自然子任务计数的阶段(disassemble、decompile)现在还会在总
  百分比之外额外带一个结构化的 `done`/`total` 字段,前端为其单独渲染一条子进度条。
  disassemble 阶段(占扫描总耗时的大头)新增实时"已处理二进制按架构分布"图,同时展示纯前端
  的动画 SVG 甜甜圈(数据一变就立即平滑动画,不用等网络往返)和仪表盘同款的 CUDA/CPU 服务端
  渲染 PNG,两者并排对照。扫描页面还加了一条纯装饰性的插画(晶圆/芯片/硬盘/通用机器人与
  机器狗图标,放大镜来回扫描),不涉及真实进度数据,`prefers-reduced-motion` 时自动关闭动画。

  **重新扫描。** 每个已完成/失败的任务行新增"重新扫描"按钮,提交时带 `force` 标志跳过去重
  缓存——直接重新提交同一路径的话会被缓存命中、原样复制回旧报告,等于白跑。`ifda.__version__`
  同步从 2.6.0 升到 2.7.0,对应上面这些报告结构的实质变化(新增仪表盘/diff 相关字段、函数
  指纹),这样去重缓存本身也能识别出升级前的旧报告已经过时。

- **2026-08-25 — 澄清:重启服务会杀掉正在跑的分析任务,不只是任务列表里看不到了。**

  此前一直假设(并据此告诉过使用者)重启 `ifda-service` 后,已经在跑的
  `python3 -m ifda.cli analyze` 子进程会作为孤儿进程继续独立跑完,只是 `Store` 里的任务记录
  随进程一起消失、UI 找不到而已——这个假设没有实测验证过。带着一个正在跑的真实分析任务实测
  重启后证伪:子进程本身也被一起终止了,已经进行的分析进度和产出全部丢失,该任务对应的
  job 目录里什么都没留下。根因还没有完全定位——`job.go` 给分析子进程设置了独立的进程组
  (`Setpgid: true`),理论上直接 `kill` 服务主进程的 PID(不带负号、不是发给整个进程组)
  不应该波及子进程,但实际观测到子进程确实被一并收掉;不排除是宿主机 systemd 用户会话把
  整个 cgroup 一起终止导致(常见于通过登录 shell 启动的后台进程——登出或会话结束会牵连整个
  cgroup 下已经重新挂到 init 的孤儿进程,`nohup` 只挡 `SIGHUP`,挡不住这种 cgroup 级别的收割)。
  **结论:重启服务前必须先确认没有正在跑的分析任务,有的话要么等它跑完,要么明确接受这个
  任务会被完全冲掉重来,不能再假设它会在后台幸存。**

- **2026-08-25 — 修复 `start.sh` 的 `-data` 指向了错误的目录,导致新任务/AI 分析看起来"丢了历史"。**

  修好上面那个 `-core` 问题重启后,AI 分析标签页的供应商配置消失,新提交的任务序号也从
  高位跳回了 job-1。原因是 `start.sh` 当时写的是 `-data ./`,而这台机器真正长期使用的生产
  数据其实一直存放在 `./.data` 子目录下——`./reports.db`(`-data ./` 实际读取的库)是一个
  远早于 AI 分析功能上线时期的旧库(连 `ai_providers` 表都不存在),任务记录也只到
  job-3,和 `./.data/reports.db` 里最新到 job-52、包含两个已配置 AI 供应商的真实生产数据
  完全是两回事。服务本身没有报错——指向一个存在但内容陈旧/为空的目录不会失败,只会安静地
  服务错误的数据,这也是这类路径参数错误比"目录不存在"更容易被忽略的原因。现在
  `start.sh` 已改为 `-data ./.data`。另外,`-user`/`-pass` 在正确的 `.data` 库下不会生效
  (`admin` 早已有密码),这属于既有的"首次播种"设计(见 `service/README.md`),不是本次
  改动引入的行为。

- **2026-08-25 — 修复 `start.sh` 用一个从未存在过的 `-core` 目录启动服务。**

  生产服务重启后所有新扫描任务立即失败,Go 层报 `fork/exec .../python3: no such file or
  directory`——这条报错具有误导性,看起来像是 python3 可执行文件本身丢了。实际原因是
  `job.go` 给子进程设置的 `cmd.Dir`(工作目录)本身不存在,子进程在 exec 之前 chdir 就失败
  了,而 Go 对 fork 阶段的任何失败都套用同一种 `fork/exec <程序路径>: <errno>` 报错格式,
  把 chdir 失败也写成了看似 python3 缺失。根因是 `service/start.sh` 里硬编码的
  `-core /root/IDA`——这台机器上从未存在过这个目录,而且不是这次改动引入的回归,从代码库
  最早的提交起这一行就是这个值;此前生产环境实际上一直是靠手动执行、带正确 `-core` 路径的
  命令启动,从未真正跑过 `start.sh`。现在改为省略 `-core`,交给 `main.go` 里已有的
  `findCoreDir()` 从当前工作目录向上查找 `ifda/__init__.py` 自动探测,和 README 里
  "`-core` 省略时自动探测"的说明保持一致,不再硬编码一个可能对不上部署环境的绝对路径。

- **2026-07-27 — v4.0:AI 分析可靠性修复 + 跨平台文档。**

  **被中断的流不再被当作已完成保存。** 一次经第三方 Anthropic 网关的分析被记为
  `status=done`,内容却停在半句:一份本应更长的报告只存下 4505 字符,断在
  `fgets() -> sub_11a8c`,且无任何错误。网关在生成中途断开了 SSE 流,既没发终结的
  `stop_reason`/`message_stop`(Anthropic),也没发 `finish_reason`/`[DONE]`(OpenAI)。
  根因:两个流解析函数都靠 `bufio.Scanner` 结束读取循环,而 `Scanner.Err()` 在 `io.EOF`
  时返回 nil——把干净的 EOF 当作正常结束。**上游在分块边界切断的流因此以"成功"返回**,带着
  已到达的部分文本和空的终结原因,于是既不拼截断提示,agent 循环也把这半截当最终答案落库。
  合规的流一定会表示它是如何结束的:现在两处都跟踪终结标记(`sawDone` / `sawMessageStop`),
  当产出了文本但既无终结原因、也无该标记时,返回新的 `errStreamInterrupted`。既有出错路径
  本就保留已流式的部分内容,所以这类运行现在会如实标为"被中断"并给出可操作的提示,而不再
  静默显示为 `done`。只发终结标记但省略原因的网关仍判为完成,避免误伤简短但正常收尾的回答。
  新增 4 个回归测试覆盖两种协议;全套测试(含 `-race`)通过。

  **一个扫描的 AI 分析不再显示另一个扫描的缓存结果。** 打开扫描 B 的 AI 分析页,可能显示的是
  扫描 A 的分析。后端按 `job_id` 取数是正确的,泄漏完全在前端视图状态:`select()` 切换任务时
  重置了 `summary`、`tab` 等按报告划分的字段,**却从不清 `aiAnalysis`**,上一个任务的分析就
  留在屏幕上;而 `loadAIAnalysis()` 在 `await` 之后无条件把结果赋回,快速切换时,已离开任务的
  慢请求会覆盖当前视图。现在 `select()` 会清 AI 视图状态——守卫在"任务确实变了"上,以免重选当前
  正在流式的任务时把对象置空(会让 delta 回调空指针);`loadAIAnalysis()` 则捕获 job id,仅当
  该任务仍被选中时才应用结果,且本标签页已拥有该任务实时流时跳过一次性 GET。

  **文档。** 明确说明平台支持:ifda 可运行于 Linux 与 macOS(Apple Silicon 与 Intel)。分析
  与宿主机架构无关(capstone 直接反汇编目标架构),Go 服务经纯 Go 的 SQLite 驱动无 cgo 原生
  构建——已通过为 `darwin/arm64` 与 `darwin/amd64` 交叉编译服务验证。新增"在 macOS 上运行"
  指南,涵盖 Homebrew 工具链、venv 安装 Python、免 cgo 构建服务、Ghidra/JDK 配置。两个 README
  同时新增了界面截图;`.gitignore` 也做了扩充。
