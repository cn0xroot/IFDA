...
- **v4.1** —— 仪表盘新增根文件系统目录占比图(CUDA/CPU 渲染,扇形内标名称+扇形外引线
  ...
  新增实时"按架构分布"图,以及跳过去重缓存的"重新扫描"按钮。
- **v4.0** —— AI 分析可靠性修复 + 跨平台文档。被中断的服务商流(没有
  `[DONE]`/`finish_reason` 或 `message_stop`/`stop_reason`)现在会如实标为"被中断"
  并保留部分输出,而不再被当作停在半句的"已完成"分析保存。修复了打开一个扫描的 AI 页却
  显示另一个扫描缓存分析的问题(切换任务时前端视图状态未清理)。文档新增 Linux + macOS
  支持说明(Apple Silicon 与 Intel)及 macOS 安装指南。
